...
function orbit_to_sigma2_profile(; r_arr, th_arr, vr_arr, xLz, r_centers_m, edges, sini)
    ...
    @inbounds for i in eachindex(r_arr)
        ...
        vφ=f64(xLz)/(rr*ss)
        vlos=f64(sini)*vr0 + sqrt(1-f64(sini)^2)*vφ
...
function launch_orbit_apocenter(; rapo::Float64, θ0::Float64, Lz_frac::Float64, pot, frc,
    ...
    Lz=Lz_frac*rapo*vc
    Φapo=pot(rapo,ss)
    !isfinite(Φapo) && return (nothing,0.0,0.0,vc,:reject_pot)

    E=Φapo + (Lz^2)/(2*rapo^2*ss^2)
    r0=r0_frac*rapo
    Φ0=pot(r0,ss)
    !isfinite(Φ0) && return (nothing,0.0,E,vc,:reject_pot0)
    ...
    if !(isfinite(arg) && arg>0)
        ...
        vr0=-sqrt(arg)
        Ω=abs(vc/r0)
        dt=dt_frac/max(Ω,1e-30)
        ((r0,θ0,dt,vr0,0.0),Lz,E,vc,:ok)
    end
end
...
function build_A_matrix_stellar(Norbit::Int, R_star_m::Vector{Float64}, v_star_mps::Vector{Float64}, verr_star_mps::Vector{Float64},
    ...
    auto_floor=max(ΔR_floor_frac*max(med_dnn,0.0), 0.02*Rmin)
    abs_floor=(ΔR_floor_pc>0) ? (ΔR_floor_pc*pc) : 0.0
    ΔR_floor=max(auto_floor,abs_floor,1e-12)
    ...
    while col<=Norbit && attempts<max_attempts
        ...
        @inbounds for istar in 1:Nstar
            ...
            ΔR=max(ΔR_frac*Ri,ΔR_floor)
            lo=Ri-ΔR; hi=Ri+ΔR
            j0=searchsortedfirst(s_sorted,lo)
            j1=searchsortedlast(s_sorted,hi)
            if j1 < j0; A_star[istar,col]=0.0; continue; end
            p=0.0; nhit=j1-j0+1
            inv_norm = inv_sqrt2π / σi
