...
# Do not force GC policy in a library module on HPC.
# Leave GC.enable alone.
...
@inline _ssin(theta::Float64) = begin
    s = sin(theta)
    abs(s) > 1e-12 ? s : safe_sign(s) * 1e-12
end

@inline function _sincos_safe(theta::Float64)
    s, c = sincos(theta)
    abs(s) > 1e-12 ? (s, c) : (safe_sign(s) * 1e-12, c)
end

@inline clamp01(x::Float64) = x < 0 ? 0.0 : (x > 1 ? 1.0 : x)

@inline function _debug_bins_enabled()
    v = get(ENV, "OSPM_DEBUG_BINS", "0")
    v == "1" || lowercase(v) == "true"
end
...
@inline function derivs(s::SVector{4,Float64}, Lz::Float64, frc, R)
    r, th, vr, vth = s
    !(isfinite(r) && isfinite(th) && isfinite(vr) && isfinite(vth)) && return SVector(0.0, 0.0, 0.0, 0.0)
    r_safe = max(abs(r), 1e-12)
    sth, cth = _sincos_safe(th)
    r_tab = clamp(r_safe, R[1], R[end])
    fr, _ = frc(r_tab, sth)
    !isfinite(fr) && return SVector(0.0, 0.0, 0.0, 0.0)
    dr  = vr
    dth = vth / r_safe
    dvr = (vth * vth) / r_safe + (Lz * Lz) / (r_safe^3 * sth * sth) + fr
    dvth = (Lz * Lz) * cth / (r_safe^3 * sth^3) - (vr * vth) / r_safe
    SVector(dr, dth, dvr, dvth)
end
...
function integrate_orbit_rk4(; ic, xLz, ctx, nsteps=4000, stop_rmin_factor=1.001)
    ...
    vth0 = length(ic) >= 5 ? f64(ic[5]) : 0.0

    state = SVector(r0, th0, vr0, vth0)
...
const _dbg_orbit_count = Threads.Atomic{Int}(0)
...
function orbit_to_sigma2_profile(; r_arr, th_arr, vr_arr, xLz, r_centers_m, edges, sini)
    ...
    k = Threads.atomic_add!(_dbg_orbit_count, 1) + 1
    do_debug = _debug_bins_enabled() && (k <= 5)
    ...
    if do_debug
        println("BIN_HITS=", hits, " rmin/max=", (rmin_seen, rmax_seen), " centers[1/end]=", (r_centers_m[1], r_centers_m[end]))
    end
    ...
    @inbounds for j in 1:nb
        if w[j] > 0
            muv = v[j] / w[j]
            sig2[j] = v2[j] / w[j] - muv^2
        end
    end
    sig2
end

function launch_orbit_apocenter(; rapo::Float64, th0::Float64, Lz_frac::Float64, pot, frc,
    r0_frac::Float64=0.98, dt_frac::Float64=0.01, dt_floor::Float64=1e-30, debug::Bool=false)

    ss = _ssin(th0)
    frs, _ = frc(rapo, ss)
    !(isfinite(frs) && isfinite(rapo) && rapo > 0) && return (nothing, 0.0, 0.0, 0.0, :reject_force)

    vc = sqrt(abs(frs) * rapo)
    !(isfinite(vc) && vc > 0) && return (nothing, 0.0, 0.0, 0.0, :reject_vc)

    Lz = Lz_frac * rapo * vc
    Phi_apo = pot(rapo, ss)
    !isfinite(Phi_apo) && return (nothing, 0.0, 0.0, vc, :reject_pot)

    E = Phi_apo + (Lz^2) / (2 * rapo^2 * ss^2)
    r0 = r0_frac * rapo
    Phi0 = pot(r0, ss)
    !isfinite(Phi0) && return (nothing, 0.0, E, vc, :reject_pot0)

    arg = 2 * (E - Phi0) - (Lz^2) / (r0^2 * ss^2)
    if !(isfinite(arg) && arg > 0)
        debug ? ((rapo, th0, Lz, arg), Lz, E, vc, :reject_turning) : (nothing, Lz, E, vc, :reject_turning)
    else
        vr0 = -sqrt(arg)
        Omega = abs(vc / r0)
        dt = dt_frac / max(Omega, 1e-30)
        ((r0, th0, dt, vr0, 0.0), Lz, E, vc, :ok)
    end
end
...
function build_A_matrix_stellar(Norbit::Int, R_star_m::Vector{Float64}, v_star_mps::Vector{Float64}, verr_star_mps::Vector{Float64},
    ...
    !(isfinite(Rmin) && isfinite(Rmax) && Rmax > Rmin) && return zeros(Float64, Nstar, Norbit)
    ...
    A_star = zeros(Float64, Nstar, Norbit)
    ...
    while col <= Norbit && attempts < max_attempts
        ...
        ic, Lz0, E0, vc, st = launch_orbit_apocenter(rapo=rapo, th0=th0, Lz_frac=f64(lf),
            pot=ctx.pot, frc=ctx.frc, r0_frac=r0_frac, dt_frac=dt_frac_orbit)
        st != :ok && continue

        r, vr, th = integrate_orbit_rk4(ic=ic, xLz=Lz0, ctx=orbit_ctx, nsteps=nsteps)
        isempty(r) && continue
        ...
        @inbounds for istar in 1:Nstar
            ...
            A_star[istar, col] = (p * inv_norm) / nhit
        end
    ...
    A = return_occ ? vcat(A_star, A_occ) : A_star
    diag ? (A, Dict{String,Any}("filled" => col - 1, "attempts" => attempts)) : A
end
...
function build_A_matrix_from_ctx(ctx::HaloContext, th0, dt, r_centers_m, valid, sini, nsteps)
    ...
    @inbounds for rapo0 in shells
        ...
        for lf in Lfrac
            r0_frac = 0.95 + 0.04 * rand(rng)
            ic, Lz0, E0, vc, st = launch_orbit_apocenter(rapo=rapo, th0=th0v, Lz_frac=f64(lf),
                pot=ctx.pot, frc=ctx.frc, r0_frac=r0_frac, dt_frac=dt_frac_orbit)

            if st != :ok
                st == :reject_turning ? (rej_turn += 1) :
                (st == :reject_force || st == :reject_vc ? (rej_force += 1) : (rej_energy += 1))
                col += 1
                continue
            end
            ...
            sig2 = orbit_to_sigma2_profile(r_arr=r, th_arr=th, vr_arr=vr, xLz=Lz0,
                                           r_centers_m=r_centers_m, edges=edges, sini=sini)
            A[:, col] .= sig2[valid]
            kept += 1

            if _debug_bins_enabled() && kept <= 8
                Emin = Inf
                Emax = -Inf
                Esum = 0.0
                nE = 0
    ...
    _debug_bins_enabled() && println("orbit columns kept = ", kept, " / ", Norbit,
        " | reject_turning=", rej_turn, " reject_energy=", rej_energy, " reject_force=", rej_force,
