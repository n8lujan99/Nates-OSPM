...
# Support code: constants, types, caches, helpers, halo physics, orbit integration,
...
function _init_orbit_work(
    Norbit::Int, R_star_m::Vector{Float64}, has_vlos::AbstractVector{Bool},
    v_star_mps::Vector{Float64}, verr_star_mps::Vector{Float64},
    sini::Float64, ctx;
    nsteps::Int, Lfrac, dt_frac_orbit::Float64, Nbins_occ::Int,
    return_occ::Bool, max_attempts_factor::Int, fill_pct::Float64,
    t_deadline::UInt64
)
    ...
    occ_edges = build_occ_edges_adaptive(R_star_m, vlos_idx)
    Nbins_occ = length(occ_edges) - 1
    ...
    return OrbitWorkState(
        Norbit, Nstar, Nvlos, Nbins_occ, Nshells, nsteps,
        max_attempts_factor, fill_target, orbit_budget, return_occ,
        f64(pi / 2), clamp01(f64(sini)), ΔR_med, Ra_match, Rb_match,
        R_star_m, v_star_mps, verr_star_mps, R_sorted, shells, vlos_idx,
        occ_edges, cost_order, orbit_ctx, ctx.pot, ctx.frc, Lfrac,
        dt_frac_orbit, t_deadline, A_vlos, A_occ, success_flags,
        min_r_reached, rapo_list, star_hit_flags,
...
function _orbit_worker!(st::OrbitWorkState, rng)
    col_occ  = zeros(Float64, st.Nbins_occ)
    col_vlos = zeros(Float64, st.Nvlos)
    s_arr    = Vector{Float64}(undef, st.nsteps)
    vlos_buf = Vector{Float64}(undef, st.nsteps)
    while true
        ...
        ic, Lz0, E0, vc, launch_state = launch_orbit_apocenter(
            rapo=rapo, theta0=st.theta0, Lz_frac=f64(lf),
            pot=st.pot, frc=st.frc, r0_frac=r0_frac, dt_frac=st.dt_frac_orbit
        )
        launch_state != :ok && continue
        r, vr, theta, vtheta = integrate_orbit_rk4(
            ic=ic, xLz=Lz0, orbit_ctx=st.orbit_ctx, nsteps=st.nsteps
        )
        isempty(r) && continue
        ...
        if st.Nvlos > 0
            ...
            @inbounds for row in 1:st.Nvlos
                istar = st.vlos_idx[row]
                Ri    = f64(st.R_star_m[istar])
                vi    = f64(st.v_star_mps[istar])
                si    = f64(st.verr_star_mps[istar])
                s2    = si * si
...
function build_A_matrix_hybrid(Norbit::Int, R_star_m::Vector{Float64}, has_vlos::AbstractVector{Bool}, v_star_mps::Vector{Float64}, verr_star_mps::Vector{Float64}, sini::Float64, rho_s::Float64, r_s::Float64, MBH::Float64, halo_type::String; nsteps::Int=DEFAULT_NSTEPS, Lfrac::NTuple{5,Float64}=DEFAULT_LFRAC, dt_frac_orbit::Float64=DEFAULT_DT_FRAC, dR_frac::Float64=DEFAULT_DR_FRAC, Nbins_occ::Int=DEFAULT_NBINS_OCC, return_occ::Bool=true, max_attempts_factor::Int=DEFAULT_MAX_ATTEMPTS, diag::Bool=false, threaded::Bool=true, fill_pct::Float64=0.95, t_deadline::UInt64=typemax(UInt64))
    ...
    if !(isfinite(Rmin) && isfinite(Rmax) && Rmax > Rmin)
        Nvlos = count(has_vlos)
        Nout  = Nvlos + (return_occ ? Nbins_occ : 0)
        return zeros(Float64, Nout, Norbit)
    end
    st = _init_orbit_work(
        Norbit, R_star_m, has_vlos, v_star_mps, verr_star_mps, sini, ctx;
        nsteps=nsteps, Lfrac=Lfrac, dt_frac_orbit=dt_frac_orbit,
        Nbins_occ=Nbins_occ, return_occ=return_occ,
        max_attempts_factor=max_attempts_factor, fill_pct=fill_pct,
    ...
    if st.Nvlos > 0
        hits = sum(st.star_hit_flags[st.vlos_idx])
        total = length(st.vlos_idx)
...
# Batch evaluator with work-stealing.
