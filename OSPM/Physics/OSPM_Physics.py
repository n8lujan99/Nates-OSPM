...
def _gcc_libstdcpp_dir():
    ...
    try:
        p = subprocess.check_output(["g++", "-print-file-name=libstdc++.so.6"], text=True).strip()
        if p and os.path.isabs(p) and os.path.exists(p): return os.path.dirname(p)
...
def _jl_init():
    ...
    if "torch" in sys.modules: print("[WARN] torch imported before juliacall. This can be unstable on some systems.")
    ...
    if not hasattr(_Main, "OSPMPhysicsSpherical"): _Main.include(jl_path)
    if not hasattr(_Main, "OSPMPhysicsSpherical"): raise RuntimeError("OSPMPhysicsSpherical failed to load into Main")
    _JL_READY = True
...
def assert_theta_contract(theta, *, halo_type, bounds=None, require_mbh=True, require_ml=True):
    ...
    if require_mbh and t.size < 3: raise ValueError("theta missing MBH (expects [rho_s, r_s, MBH, ML])")
    if require_ml and t.size < 4: raise ValueError("theta missing ML (expects [rho_s, r_s, MBH, ML])")
    ncheck = 4 if require_ml else 3
    if not np.all(np.isfinite(t[:ncheck])): raise ValueError("theta has non-finite values")
    rho_s = float(t[0])
    r_s   = float(t[1])
    MBH   = float(t[2]) if t.size >= 3 else 0.0
    ML    = float(t[3]) if t.size >= 4 else 1.0
    ...
    if ML < 0: raise ValueError("theta expects ML>=0")
    ht = str(halo_type).strip().lower()
    ...
    if bounds is not None:
        ...
        if b.shape[0] < need: raise ValueError(f"bounds must cover at least first {need} parameters")
...
def mass_enclosed_two_radii_julia(*, r_in_m, r_out_m, theta, halo_type, stellar_model=None):
    ...
    Min, Mout = _Main.OSPMPhysicsSpherical.mass_enclosed_two_radii(
        float(r_in_m), float(r_out_m), float(rho_s), float(r_s), float(MBH), float(ML), str(ht),
...
def build_dynamics_context(*, theta, halo_type, stellar_model=None, **_ignored):
    if not USE_JULIA: raise RuntimeError("Python backend is disabled. Set OSPM_USE_JULIA=1.")
    return {"halo": halo_from_theta_astro(theta, halo_type=halo_type), "stellar_model": stellar_model}
...
def halo_kwargs_from_ctx(ctx):
    ...
    return dict( rho_s=float(halo["rho_s"]), r_s=float(halo["r_s"]), MBH=float(halo["MBH"]), ML=float(halo["ML"]), halo_type=str(halo["type"]),)


def build_A_matrix_stellar_julia(*, R_star_m, v_star_mps, verr_star_mps, sini, Norbit, theta, halo_type, stellar_model=None, return_occ=False, Nbins_occ=6, diag=False):
    if not USE_JULIA: raise RuntimeError("Stellar mode requires Julia")
    _jl_init()
    rho_s, r_s, MBH, ML, ht = assert_theta_contract(theta, halo_type=halo_type, require_mbh=True, require_ml=True)
    maybe_reset_orbit_cache((rho_s, r_s, MBH, ML), ht)
    PC = _Main.PythonCall; VecF = _Main.Vector[_Main.Float64]
    R_py = np.asarray(R_star_m, dtype=float); v_py = np.asarray(v_star_mps, dtype=float); ve_py = np.asarray(verr_star_mps, dtype=float)
    Rj = PC.pyconvert(VecF, R_py); vj = PC.pyconvert(VecF, v_py); vej = PC.pyconvert(VecF, ve_py)
    out = _Main.OSPMPhysicsSpherical.build_A_matrix_stellar(
        int(Norbit), Rj, vj, vej, float(sini), float(rho_s), float(r_s), float(MBH), float(ML), str(ht),
        stellar_model=stellar_model, return_occ=bool(return_occ), Nbins_occ=int(Nbins_occ), diag=bool(diag)
    )
    if diag:
        A, meta = out
...
def build_A_matrix(obs, ctx, *, return_occ=False, Nbins_occ=6, diag=False):
    ...
    hk = halo_kwargs_from_ctx(ctx); theta = [hk["rho_s"], hk["r_s"], hk["MBH"], hk["ML"]]; halo_type = hk["halo_type"]
    stellar_model = ctx.get("stellar_model", getattr(obs, "stellar_model", None)) if isinstance(ctx, dict) else getattr(ctx, "stellar_model", getattr(obs, "stellar_model", None))
    ...
    R = np.asarray(obs.R_star_m, float)[vv]; v = np.asarray(obs.v_star_mps, float)[vv]; ve = np.asarray(obs.verr_star_mps, float)[vv]
    if len(R) == 0: raise RuntimeError("No RV-valid stars to build A-matrix")
    return build_A_matrix_stellar_julia(
        R_star_m=R, v_star_mps=v, verr_star_mps=ve, sini=float(obs.sini), Norbit=int(obs.Norbit),
        theta=theta, halo_type=halo_type, stellar_model=stellar_model,
...
def build_A_matrix_from_theta(obs, theta, *, halo_type="nfw", return_occ=True, Nbins_occ=6, diag=False):
    ctx = build_dynamics_context(theta=theta, halo_type=halo_type, stellar_model=getattr(obs, "stellar_model", None))
