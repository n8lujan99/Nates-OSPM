...
def _gcc_libstdcpp_dir():
    # returns directory containing libstdc++.so.6 (from loaded gcc module), or None
...
def _jl_init():
    ...
    # juliacall warns if torch is already imported (can segfault)
    # enforce the safer order deterministically
    if "torch" in sys.modules:
    	print("[WARN] torch imported before juliacall. This can be unstable on some systems.")

    # re-assert project for safety (must be set before importing juliacall)
...
def assert_theta_contract(theta, *, halo_type, bounds=None, require_mbh=True):
    ...
    if require_mbh and t.size < 3:
        raise ValueError("theta missing MBH (expects [rho_s,r_s,MBH])")
    ...
    if rho_s <= 0 or r_s <= 0:
        raise ValueError("theta expects rho_s>0 and r_s>0")
    if MBH < 0:
        raise ValueError("theta expects MBH>=0")
    ...
    if bounds is not None:
        ...
        if b.shape[0] < 3:
            raise ValueError("bounds must cover at least rho_s,r_s,MBH")
...
def mass_enclosed_two_radii_julia(*, r_in_m, r_out_m, theta, halo_type):
    ...
    Min, Mout = _Main.OSPMPhysicsSpherical.mass_enclosed_two_radii(
        float(r_in_m), float(r_out_m), float(rho_s), float(r_s), float(MBH), str(ht)
    )
...
def build_A_matrix(obs, ctx, *, return_occ=False, Nbins_occ=6, diag=False):
    ...
    if mode != "stellar":
        raise RuntimeError("build_A_matrix only supports obs.mode=='stellar'")
    ...
    if vv is None:
        ...
        if has is None:
            raise RuntimeError("obs missing valid_vlos (or has_vlos)")
        vv = (
            has
            & np.isfinite(obs.v_star_mps)
