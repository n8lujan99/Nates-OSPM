...
def wrap_physics_engine(base_engine, *, obs, halo_type, config=None):
    ...
    if R_star_m is None or v_star_mps is None:
        raise AttributeError("obs must expose R_star_m and v_star_mps")

    v_star_mps = np.asarray(v_star_mps, float)
    if verr_star_mps is not None:
        verr_star_mps = np.asarray(verr_star_mps, float)
    ...
    def engine(theta):
        ...
        if (_print_counter % 50) == 0:
            print("[PHYS] base_engine type:", type(A))
