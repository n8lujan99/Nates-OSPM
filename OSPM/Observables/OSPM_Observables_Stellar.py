...
class OSPMObservablesStellar:
    ...
    def from_star_table(cls, csv_path, *, r_col="r_pc", v_col="vlos", 
        ...
        return cls( R_star_pc=df[r_col].values, v_star_kms=df[v_col].values, verr_star_kms=df[verr_col].values, 
                has_vlos=df[has_vlos_col].values, inclination_deg=inclination_deg, Norbit=Norbit )
